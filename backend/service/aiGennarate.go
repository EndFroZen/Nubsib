...
คำตอบ:`,
	...
	result, err := llms.GenerateFromSinglePrompt(ctx, llm, finalPrompt,
	...
	FLog("5. SQL Safety Validation")
	sqlQuery := ExtractSQL(result)
	if sqlQuery != "" {
		if err := ValidateSQL(sqlQuery); err != nil {
			FLog("🚫 SQL Blocked!", sqlQuery, err.Error())
			return "", fmt.Errorf("🚫 SQL ไม่ปลอดภัย: %v", err)
		}
		FLog("✅ SQL Validated — SELECT only", sqlQuery)
	} else {
		FLog("⚠️ ไม่พบ SQL ใน response — ส่งผลลัพธ์กลับตามปกติ")
	}
